...
- сделан пятый микросервис PostPaymentApp
- из Kafka вычитываются события по оплате, в отдельном потоке идет обработка, которая
занимает от 30 секунд по 5 минут, после чего мы сохраняем событие об оплате и кладем его
в отдельный топик payment-processed
